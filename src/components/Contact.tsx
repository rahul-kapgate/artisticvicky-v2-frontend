...
const Contact: React.FC = () => {
  return (
    <section
      ...
      style={{
        background: "linear-gradient(to bottom, #050816, #10194f, #1a237e, #10194f, #050816)",
      }}
    >
      {/* Animated blobs */}
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-slate-400 max-w-md mx-auto text-base leading-relaxed">
            Reach out through any platform — whether you want to learn, collaborate, or just say hi!
          </p>
          ...
          {contacts.map((item) => (
            ...
            >
              <div
                ...
                style={{ background: item.iconColor, border: `1px solid ${item.iconBorder}` }}
              >
                {item.icon}
              </div>
              <div className="min-w-0">
                <p className="font-semibold text-sm text-slate-200">{item.label}</p>
                <p className="text-xs text-slate-400 break-all leading-snug mt-0.5">{item.text}</p>
              </div>
            </a>
          ))}
        </div>
